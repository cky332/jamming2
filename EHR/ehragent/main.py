...
def get_unique_file_path(file_path):
    """Appends a number to the file path if a file with the same name exists."""
...
def main():
    ...
    parser.add_argument("--save_memory_path", type=str, default=None, help="Path to save the long-term memory")
    parser.add_argument("--load_memory_path", type=str, default=None, help="Path to load the long-term memory")
    args = parser.parse_args()
    set_seed(args.seed)

    # Load memory if specified, otherwise initialize it
    if args.load_memory_path:
        long_term_memory = load_memory(args.load_memory_path)
    else:
        long_term_memory = []
    ...
    # Initialize with loaded or default memory
    ...
    # Random shuffle
    import random
    # Comment below to disable random shuffle
    ...
    for i in range(args.start_id, args.num_questions):
        ...
        file_directory = get_unique_file_path(file_directory)  # Ensure unique file name
    ...
    if args.save_memory_path:
        save_memory(long_term_memory, args.save_memory_path)
